...
def display_barrier(xlocation,randomHeight,barrierWidth):
	pygame.draw.rect(gameDisplay,green,(xlocation,display_height-randomHeight,barrierWidth,randomHeight))

def missile_shoot(x,y,turretAngle=math.radians(45),power=0.01):
	...
	while fire:
		...
		startMissile[1] += int((((startMissile[0]-xy[0])*0.01)**2) - (int(turretAngle)+int(turretAngle)/(12-int(turretAngle))))
		pygame.display.update()
		if startMissile[1]>display_height:
			fire = False
		clock.tick(60)
...
def game_loop():
	...
	while not gameExit:
		...
		for event in pygame.event.get(): 	#event handling loop
			...
			if event.type == pygame.KEYDOWN:
				if event.key == pygame.K_LEFT:
					...
					missile_shoot(mainTankX,mainTankY,turretAngle)
		...
		elif mainTankX - tankWidth <= xlocation + barrierWidth:	#restriction for barrier boundary
