...
def display_enemyTank(x,y,turretAngle=math.radians(45)):
	x = int(x)
	y = int(y)
	# drawing turret
...
def missile_shoot(x,y,power,turretAngle=math.radians(45),player="main"):
	...
	while fire:
		...
		if startMissile[0]>=xlocation and startMissile[0]<=xlocation+barrierWidth and startMissile[1]<=display_height and startMissile[1]>=display_height-randomHeight:
			...
			if player == "main":
				...
				gameDisplay.blit(enemy_barrier_blast,[xlocation-40,startMissile[1]])	#40 is blast image width
			pygame.display.update()
			time.sleep(0.5)
		clock.tick(60)
...
def game_loop():
	...
	while not gameExit:
		...
		for event in pygame.event.get(): 	#event handling loop
			...
			if event.type == pygame.KEYDOWN:
				...
				if event.key == pygame.K_a:
					...
					missile_shoot(enemyTankX,enemyTankY,power,enemyTurretAngle,"enemy")
		...
		#display enemy tank
		display_enemyTank(enemyTankX,enemyTankY,enemyTurretAngle)
