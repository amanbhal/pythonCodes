# adding and eating the apple and increasing the length of the snake
import pygame
import time
import random
...
def game_loop():
	...
	while not gameExit:
		...
		gameDisplay.fill(red, rect=[appleX,appleY,block_size,block_size])
		...
		if lead_x == appleX and lead_y == appleY:	#generate a new apple wenever we eat an apple
			appleX = round(random.randrange(0,display_width-block_size)/10.0)*10.0
			appleY = round(random.randrange(0,display_height-block_size)/10.0)*10.0
			snakelength += 1
