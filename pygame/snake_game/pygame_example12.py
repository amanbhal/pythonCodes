#adding game intro screen
#adding apple image
#adding game icon image
#adding the score
import pygame
import time
import random
...
def display_snake(block_size,snakelist):
	if direction == "right":
		head = pygame.transform.rotate(snake_img,270)	#rotate img which is up aligned to watever angle we want in anti-clockwise direction
	elif direction == "left":
...
def game_intro():
		...
		while intro:
			...
			message_to_screen("APPLE HUNT",red,y_displace=-100,size="large")
			message_to_screen("Collect all the apples and don't run into screen boundary or yourself",white)
...
def game_loop():
	...
	block_size = 20
	appleSize = 30 	#making apple bigger
	snakelist = []
	snakelength = 1
	...
	while not gameExit:
		...
		while gameOver == True:
			...
			message_to_screen("Press C to continue or Q to quit",white,y_displace=50)
			pygame.display.update()
